...
// Send mechanics intentionally mirror trigger/drip-dispatch.ts: both deliver an
// approved template through the same two providers, and a divergent second copy
// would drift.
...
type BulkMessage = {
  ...
  campaign_id: string;
  asset_object_file_id: string | null;
  lease_token: string;
};
...
async function sendWhatsApp(
  ...
  if (!message.connected_account_id) throw new Error('BULK_WHATSAPP_NOT_CONNECTED');
  const { data: secret, error } = await supabase
    ...
    .maybeSingle();
  if (error || !secret) throw error ?? new Error('BULK_WHATSAPP_CREDENTIAL_NOT_FOUND');
  ...
  if (recipient.length < 7 || recipient.length > 20) throw new Error('BULK_RECIPIENT_INVALID');
  ...
  if (!response.ok || !payload?.messages?.[0]?.id)
    throw new Error(
      response.status === 429 ? 'BULK_WHATSAPP_RATE_LIMITED' : 'BULK_WHATSAPP_REJECTED',
    );
  return payload.messages[0].id;
}

async function sendEmail(message: BulkMessage): Promise<string> {
  const recipient = String(message.recipient ?? '').trim();
  if (!/^[^@\s]+@[^@\s]+\.[^@\s]+$/.test(recipient)) throw new Error('BULK_RECIPIENT_INVALID');
  const response = await fetch('https://api.brevo.com/v3/smtp/email', {
    ...
    body: JSON.stringify({
      ...
      tags: ['go-digital-crm', 'bulk-campaign'],
    }),
    signal: AbortSignal.timeout(30_000),
  });
  const payload = (await response.json().catch(() => null)) as { messageId?: string } | null;
  if (!response.ok || !payload?.messageId)
    throw new Error(response.status === 429 ? 'BULK_EMAIL_RATE_LIMITED' : 'BULK_EMAIL_REJECTED');
  return payload.messageId;
}

async function deliver(supabase: ReturnType<typeof createClient>, message: BulkMessage) {
  ...
  if (!message.template_provider_id) throw new Error('BULK_TEMPLATE_NOT_APPROVED');
  if (!message.recipient) throw new Error('BULK_RECIPIENT_MISSING');
