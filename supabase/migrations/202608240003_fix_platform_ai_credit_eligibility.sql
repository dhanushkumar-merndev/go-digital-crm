-- Expose lifecycle eligibility alongside immutable platform AI-credit history.
-- The grant RPC remains the authoritative status check at mutation time.
...
begin
  ...
  with period_ledger as materialized (
    ...
    select organization_row.id, organization_row.name, organization_row.status
