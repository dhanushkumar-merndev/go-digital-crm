...
-- 202609020006 introduced a runtime failure in create_lead: its local variable
-- `normalized_phone` and public.customers.normalized_phone are both in scope in
-- the customer lookup, so every Add Lead raised
-- `column reference "normalized_phone" is ambiguous` (42702). plpgsql resolves
-- that at execution, not at creation, so the migration applied cleanly and the
-- break only showed up under `supabase db lint`.
--
-- The digits now live in their own local, which removes the collision and
-- computes them once rather than once per compared row.
