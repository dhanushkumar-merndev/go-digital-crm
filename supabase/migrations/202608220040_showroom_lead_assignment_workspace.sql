...
-- Showroom Managers use the same audited public.assign_lead transaction as Team
-- Managers. This RPC only exposes the unassigned queues in branches already
-- allowed by their data scope.
...
declare
  ...
  result jsonb;
begin
  if target_page not between 1 and 100000
    or target_page_size not in (25, 50, 100)
    or length(normalized_search) > 160
  then
    raise exception using errcode = '22023', message = 'INVALID_SHOWROOM_ASSIGNMENT_QUERY';
  end if;
  ...
  then
    raise exception using errcode = '42501', message = 'SHOWROOM_ASSIGNMENT_ACCESS_REQUIRED';
  end if;
  ...
  if not app_private.has_permission(current_organization_id, 'lead.assign') then
    raise exception using errcode = '42501', message = 'SHOWROOM_LEAD_ASSIGN_PERMISSION_REQUIRED';
  end if;
  ...
  if cardinality(allowed_branch_ids) = 0 then
    raise exception using errcode = '42501', message = 'SHOWROOM_BRANCH_SCOPE_REQUIRED';
  end if;
  ...
  if cardinality(scoped_team_ids) = 0 then
    raise exception using errcode = 'P0002', message = 'SHOWROOM_ASSIGNMENT_TEAMS_NOT_FOUND';
  end if;
  ...
  with queue as materialized (
    ...
    where lead_row.organization_id = current_organization_id
      ...
      and lead_row.team_id = any(scoped_team_ids)
    ...
    select id, team_id, customer_id, customer_name, phone, source, interested_model,
      lifecycle_status, temperature, created_at, team_name, assignment_kind, assignment_mode
    from queue
    ...
    select
      ...
      count(lead_row.id) filter (
        where lead_row.deleted_at is null and lead_row.lifecycle_status <> 'Lost'
      )::bigint as current_leads,
      count(lead_row.id) filter (
        where lead_row.deleted_at is null
    ...
    where member_row.organization_id = current_organization_id
      and member_row.team_id = any(scoped_team_ids)
    ...
    where history_row.organization_id = current_organization_id
      and history_row.team_id = any(scoped_team_ids)
    order by history_row.created_at desc, history_row.id desc
    limit 25
  )
  select jsonb_build_object(
    'total', (select count(*)::integer from queue),
    'records', coalesce((select jsonb_agg(jsonb_build_object(
      'id', row.id, 'team_id', row.team_id, 'team_name', row.team_name,
      'customer_id', row.customer_id, 'customer_name', row.customer_name, 'phone', row.phone,
      'source', row.source, 'interested_model', row.interested_model,
      'lifecycle_status', row.lifecycle_status, 'temperature', row.temperature,
      'created_at', row.created_at, 'assignment_kind', row.assignment_kind,
    ...
    'consultants', coalesce((select jsonb_agg(jsonb_build_object(
      'user_id', row.user_id, 'team_id', row.team_id, 'full_name', row.full_name,
      'eligible_for_fresh_leads', row.eligible_for_fresh_leads,
      'eligible_for_qualified_leads', row.eligible_for_qualified_leads,
      'current_leads', row.current_leads, 'hot_leads', row.hot_leads
    ...
    'recent_assignments', coalesce((select jsonb_agg(jsonb_build_object(
      ...
      'assigned_to_name', row.assigned_to_name, 'assigned_by_name', row.assigned_by_name,
      'method', row.method, 'reason', row.reason, 'created_at', row.created_at
...
revoke all on function public.get_showroom_lead_assignment_workspace(text, integer, integer)
from public, anon;
grant execute on function public.get_showroom_lead_assignment_workspace(text, integer, integer)
to authenticated;
