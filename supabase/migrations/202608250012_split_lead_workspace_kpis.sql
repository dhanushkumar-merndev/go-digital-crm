...
-- Paging re-ran seventeen aggregates plus a count over the entire scoped lead
-- set on every request: the page itself was already indexed and cheap, but the
-- tab counters were not, so clicking to page two rebuilt every counter. Those
-- counters only move when a filter moves, never when the offset does, so the
-- two halves are now requested separately and cached on separate keys.
--
-- The new trailing defaults widen the signature, and `create or replace` would
-- register an overload rather than replace: every existing call would then be
-- ambiguous. Both old signatures are dropped first.
drop function if exists public.get_lead_workspace_page_v2(
...
create or replace function app_private.get_sales_role_lead_workspace_page(
  ...
  target_include_records boolean default true,
  target_include_kpis boolean default true
)
returns jsonb
language plpgsql
stable
...
begin
  ...
  return (
    ...
    select jsonb_build_object(
      ...
      'total', case
        when target_include_kpis then (select count(*) from filtered_lead_ids)
        else null
      end,
      'kpis', case when target_include_kpis then (select to_jsonb(kpis) from kpis) else null end,
      'filters', case when not target_include_kpis then null else jsonb_build_object(
        'models', coalesce((
...
begin
  ...
  if access_context->>'role_key' = any(array[
    ...
    return app_private.get_sales_role_lead_workspace_page(
      ...
      target_followup_to,
      target_include_records,
      target_include_kpis
    );
  end if;

  -- The legacy implementation predates the split and always computes both
  -- halves; correctness is unaffected, only the saving is.
...
revoke all on function app_private.get_sales_role_lead_workspace_page(
  uuid, uuid, boolean, uuid[], uuid[], boolean, uuid[], integer, integer,
  text, text, text, text, text, text, text, date, date, boolean, boolean
) from public, anon, authenticated;

revoke all on function public.get_lead_workspace_page_v2(
  integer, integer, text, text, text, text, text, text, text, date, date, boolean, boolean
) from public, anon;
