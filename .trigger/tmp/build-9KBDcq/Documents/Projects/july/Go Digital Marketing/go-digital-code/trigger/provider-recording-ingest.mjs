  ...
  "node_modules/.pnpm/@aws-sdk+lib-storage@3.1110.0_@aws-sdk+client-s3@3.1110.0/node_modules/@aws-sdk/lib-storage/dist-cjs/index.js"(exports) {
    ...
    var byteLength = /* @__PURE__ */ __name((input) => {
      ...
      if (typeof input === "string") {
        return Buffer2.byteLength(input);
      }
    ...
    async function* getChunkStream(data, partSize, getNextData) {
      ...
      for await (const datum of getNextData(data)) {
        ...
        while (currentBuffer.length > partSize) {
          const dataChunk = currentBuffer.chunks.length > 1 ? Buffer2.concat(currentBuffer.chunks) : currentBuffer.chunks[0];
          yield {
            partNumber,
      ...
      yield {
        partNumber,
        data: currentBuffer.chunks.length !== 1 ? Buffer2.concat(currentBuffer.chunks) : currentBuffer.chunks[0],
        lastPart: true
      };
    }
    ...
    var getChunk = /* @__PURE__ */ __name((data, partSize) => {
      ...
      if (data instanceof String || typeof data === "string") {
        return getChunkUint8Array(Buffer2.from(data), partSize);
      }
...
async function fetchRecording(initialUrl, allowedHosts, signal, headers = {}) {
...
var providerRecordingIngest = task({
  ...
  run: /* @__PURE__ */ __name(async (payload) => {
    ...
    if (payload.provider === "twilio_voice") {
      ...
      const { data: credentialRow, error: credentialError } = await supabase.from("integration_credentials").select("encrypted_payload").eq("organization_id", payload.organizationId).eq("connected_account_id", payload.connectionId).maybeSingle();
      if (credentialError || !credentialRow)
        throw credentialError ?? new Error("TWILIO_CREDENTIAL_NOT_FOUND");
      const credential = await decryptCredential(credentialRow.encrypted_payload);
      providerHeaders = {
